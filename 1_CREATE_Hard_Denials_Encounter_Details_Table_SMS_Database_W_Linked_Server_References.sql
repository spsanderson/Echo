...
SELECT [pa-pt-no],
coalesce([pa-unit-date],[pa-dsch-date]) as 'Svc_Date',
YEAR(coalesce([pa-unit-date],[pa-dsch-date])) as 'Svc_Year',
YEAR([pa-dtl-post-date]) as 'WO_Year',
CAST(a.[pa-pt-no] as varchar)+CAST(coalesce([pa-unit-date],[pa-dsch-date])as varchar),
[pa-dtl-svc-cd],
[pa-dtl-technical-desc],
[pa-dtl-chgs]


FROM [SMS].[dbo].[Denied_Encounter_Details] a
