...
------------------------------------------------------------------------------------------------------------------------------------------------------
 /*Create Jzanus Denied Placement Temp Table*/
...
INSERT INTO #JzanusDenied ([PA-PT-NO],[JZANUS-IND],[JZANUS-COMMENT])
...
--,CAST(m.[pa-pt-no-woscd] as varchar) + CAST(m.[pa-pt-no-scd] as varchar)
...
left outer join dbo.[UserDefined] m
...
--a.[pa-acct-type] NOT IN ('4','6')
 ...
 COALESCE(DATEADD(DAY,1,EOMONTH(b.[pa-unit-date],-1)),a.[pa-adm-date])  BETWEEN '2017-05-01 00:00:00.000' AND '2017-06-14 23:59:59.000'
 AND c.[pa-ins-co-cd]='H'
...
AND (a.[pa-pt-type]='B' or (a.[pa-pt-type]='I' and a.[pa-pt-name] LIKE '%BABYB%') or (a.[pa-pt-type]='I' and a.[pa-pt-name] LIKE '%BABYG%'))
