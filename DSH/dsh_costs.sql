...
IF EXISTS [temp_DSH_Costs];
	DROP TABLE

IF EXISTS [temp_DSH_Costs2];
	DROP TABLE

IF EXISTS [2016_DSH_Costs];GO
...
SELECT COSTS.[pa-pt-no-woscd],
	...
	COALESCE(RPTGRP.[REPORTING GROUP], RPTGRP2.[REPORTING GROUP]) AS 'REPORTING GROUP'
INTO [2016_DSH_Costs]
FROM [DSH].[dbo].[temp_DSH_Costs2] AS COSTS
LEFT OUTER JOIN DBO.[DSH_INSURANCE_TABLE_W_REPORT_GROUPS] AS RPTGRP ON COSTS.[PA-PT-NO-WOSCD] = RPTGRP.[PA-PT-NO-WOSCD]
	AND COSTS.[PA-PT-NO-SCD] = RPTGRP.[PA-PT-NO-SCD]
	AND COSTS.[PA-UNIT-NO] = RPTGRP.[pa-unit-no]
	AND COSTS.[UNIT-DATE] = RPTGRP.[pa-unit-date]
LEFT OUTER JOIN dbo.[DSH_INSURANCE_TABLE_W_REPORT_GROUPS] AS RPTGRP2 ON COSTS.[PA-PT-NO-WOSCD] = RPTGRP2.[PA-PT-NO-WOSCD]
	AND COSTS.[PA-PT-NO-SCD] = RPTGRP2.[PA-PT-NO-SCD]
	AND COSTS.[PA-UNIT-NO] IS NULL
	AND COSTS.[UNIT-DATE] IS NULL
