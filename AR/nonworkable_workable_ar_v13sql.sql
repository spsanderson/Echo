...
SELECT A.PT_NO,
	...
	DEPARTMENT_CLEAN = CASE
		...
		WHEN DEPARTMENT_UPDATED IS NULL
			AND DEPARTMENT = 'BAD DEBT VENDORS'
			AND PT_REPRESENTATIVE IN ('780','781','782','783')
				THEN 'KOPP'
		WHEN DEPARTMENT_UPDATED IS NULL
			AND BUCKET_INS_INDICATOR = 'PT BALANCE'
	...
	[WORKABLE_INDICATOR] = CASE
		...
		WHEN DEPARTMENT_UPDATED = 'NON-GOVERNMENTAL FOLLOW UP'
			...
			AND WORKLISTED = 'NO'
			THEN 'WORKABLE_PENDING'
		WHEN DEPARTMENT_UPDATED IN ('GOVERNMENTAL-NO WORK')
			THEN 'WORKABLE_PENDING'
...
SELECT [PT_NO] = A.PT_NO,
	...
	[INS4_CD] = A.Ins4_Cd,
	A.WORKLISTED,
	A.ASSIGNED_WORKLIST_ID,
	[WORKLIST_NAME] = A.[Worklist Name],
	[PT_REPRESENTATIVE] = A.Pt_Representative,
	[ACCT_TYPE] = A.Acct_Type,
	A.BUCKET_INS_INDICATOR,
	A.BUCKET_INS_CODE,
	A.BUCKET_BALANCE,
	A.BUCKET_PAYER_HEALTH_PLAN,
	A.BUCKET_INS_CODE_LAST_BILLED_DATE,
	A.BUCKET_INS_CODE_LAST_BILLED_DATE,
	A.BUCKET_INS_CODE_LAST_FOLLOW_UP_DATE,
	A.ACCT_LAST_FOLLOW_UP_DATE,
	A.CLAIM_DELETED_IND,
	B.DEPARTMENT,
	B.INITIAL_DEPARTMENT,
	B.DEPARTMENT_UPDATED,
	B.DEPARTMENT_CLEAN,
	[NEW_DEPT_CLEAN] = A.DEPARTMENT_CLEAN,
	B.WORKABLE_INDICATOR,
	[NEW_WORKABLE_INDICATOR] = CASE 
		-- ✅ 1. Recently billed stays first (preserve your intent)
		WHEN A.DEPARTMENT_UPDATED IN ('RECENTLY BILLED NON-GOVERNMENTAL PENDING', 'RECENTLY BILLED NON-GOVERNMENTAL UNITIZED FOLLOW UP PENDING', 'RECENTLY BILLED GOVERNMENTAL PENDING', 'RECENTLY BILLED SPECIAL BILLING', 'GOVERNMENTAL RECENTLY QUALIFIED SECONDARY BALANCES', 'NON-GOVERNMENTAL RECENTLY QUALIFIED SECONDARY BALANCES', 'SELF PAY DELETED FROM SCRUBBER', 'SELF PAY SCRUBBER')
			THEN 'RECENTLY_BILLED'
				-- ✅ 2. INHOUSE / DNFB stays isolated
		WHEN A.DEPARTMENT_UPDATED IN ('INHOUSE', 'DNFB', 'OP OPEN UNIT')
			THEN 'INHOUSE_DNFB'
				-- ✅ 3. SCRUBBER → Needs analyst review (preserve your v12 change)
		WHEN A.DEPARTMENT_UPDATED LIKE '%SCRUBBER%'
			THEN 'NEEDS_ANALYST_REVIEW'
				-- ✅ 4. PRIMARY NEW RULE: WORKABLE PENDING
		WHEN A.WORKLISTED = 'NO'
			AND LEFT(A.PT_REPRESENTATIVE, 1) NOT IN ('5', '6', '7')
			--AND PT_REPRESENTATIVE NOT IN ('000','0', '   ')
			THEN 'WORKABLE_PENDING'
				-- ✅ 5. PRIMARY NEW RULE: WORKABLE_PENDING (rep empty + has follow-up)
		WHEN A.WORKLISTED = 'NO'
			AND A.PT_REPRESENTATIVE NOT IN ('000', '0', '   ')
			AND A.ACCT_LAST_FOLLOW_UP_DATE IS NOT NULL
			THEN 'WORKABLE_PENDING'
				-- ✅ 6. Department-driven pending (your existing behavior)
		WHEN A.DEPARTMENT_UPDATED LIKE '%PENDING%'
			THEN 'WORKABLE_PENDING'
				-- ✅ 7. If on worklist → WORKABLE
		WHEN A.WORKLISTED = 'YES'
			THEN 'WORKABLE'
				-- ✅ 8. Catch-all NOT WORKABLE
		WHEN A.WORKLISTED = 'NO'
			THEN 'NOT_WORKABLE'
		ELSE 'NEEDS_REVIEW'
		END
FROM #FINAL AS A
LEFT JOIN SMS.DBO.C_WORKABLE_INDICATOR_DETAIL_TBL AS B ON A.Pt_No = B.PT_NO
	AND (
		A.Unit_Date = B.UNIT_DATE
		OR (
			A.Unit_Date IS NULL
			AND B.UNIT_DATE IS NULL
			)
		)
	AND B.Report_Run_Date = (
		SELECT MAX(REPORT_RUN_DATE)
		FROM SMS.dbo.C_WORKABLE_INDICATOR_DETAIL_TBL
		)
WHERE A.DEPARTMENT_CLEAN = 'KOPP GROUP'
ORDER BY B.DEPARTMENT_CLEAN;
...
SELECT *
FROM #FINAL2
WHERE DEPARTMENT_CLEAN = 'JZANUS MEDICAID APPLICATIONS'
*/