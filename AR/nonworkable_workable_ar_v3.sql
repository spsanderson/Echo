...
DROP TABLE IF EXISTS #WORK_NOWORK_TBL;
	...
	WHERE TOT_CHGS > 0
		...
		AND (
			...
			OR (
				BALANCE = 0
				AND COALESCE([Worklist on Ins1], [Worklist on Ins2], [Worklist on Ins3], [Worklist on Ins4]) IS NOT NULL
			)
		)
		AND FC != 'X'
		AND PT_TYPE != 'K'
	...
	SELECT A.Pt_No,
		...
		DEPARTMENT = CASE 
			...
			WHEN A.WORKABLE_NOTWORKABLE_IND = 'NOT_WORKABLE'
                ...
                AND LEFT(A.[BUCKET_INS_CODE], 1) NOT IN ('I','L','Q')
			...
			WHEN A.WORKABLE_NOTWORKABLE_IND = 'NOT_WORKABLE'
				AND LEFT(A.[BUCKET_INS_CODE], 1) IN ('C', 'G', 'J', 'M', 'N', 'O', 'P', 'U', 'Y')
				AND A.[Ins1_Last_BL_Date] < DATEADD(day, - 45, dateadd(wk, datediff(wk, 0, getdate()), - 2))
				AND A.[LAST ACTIVITY DATE] > DATEADD(day, - 30, dateadd(wk, datediff(wk, 0, getdate()), - 2))
			...
			WHEN A.WORKABLE_NOTWORKABLE_IND = 'NOT_WORKABLE'
				...
				THEN 'RESEARCH'
			WHEN LEFT(A.[BUCKET_INS_CODE], 1) IN ('I', 'L', 'Q')
...
SELECT A.Pt_No,
	...
	DEPARTMENT = CASE 
		...
		WHEN A.DEPARTMENT IS NULL
			...
			THEN 'SCRUBBER'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization NOT IN ('EMPLOYEE HEALTH', 'INSTITUTIONAL', 'MEDICAID FFS', 'MEDICARE FFS', 'NO FAULT', 'RESEARCH', 'SELF PAY', 'SPECIAL', 'WORKERS COMPENSATION', 'TRICARE', 'VA CCN', 'WORLD TRADE CENTER')
		...
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('TRICARE', 'VA CCN', 'WORLD TRADE CENTER')
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
			THEN 'NON-GOVERNMENTAL FOLLOW UP PENDING'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('MEDICAID FFS', 'MEDICARE FFS')
			AND Pyr1_Pay_Amt IS NULL
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
		...
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('INSTITUTIONAL', 'SPECIAL')
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
			THEN 'SPECIAL BILLING'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('NO FAULT', 'WORKERS COMPENSATION')
			AND Pyr1_Pay_Amt IS NULL
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
		...
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('EMPLOYEE HEALTH')
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
			THEN 'EMPLOYEE HEALTH-NO WORK'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PRIMARY'
			AND A.payer_organization IN ('RESEARCH')
			AND Pt_Representative IN ('', '0', '000')
			AND IFPJOC IS NULL
		...
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND LEFT(A.[BUCKET_INS_CODE], 1) IN ('L', 'I', 'Q')
			AND A.[BUCKET_INS_CODE] NOT IN ('L60', 'L45')
			THEN 'SPECIAL BILLING PENDING'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND LEFT(A.[BUCKET_INS_CODE], 1) IN ('A', 'B', 'D')
			THEN 'GOVERNMENTAL-NO WORK'
		WHEN A.DEPARTMENT = 'PFS-NOWORK'
			AND A.INS_BUCKETS = 'PT BALANCE'
			THEN 'SELF PAY AND FINANCIAL ASSIST.'
		WHEN A.DEPARTMENT = 'ADMINISTRATION'
			AND A.[RESPONSIBLE COLLECTOR] = 'ADESTE'
			THEN 'AUDIT COORDINATOR'
		WHEN A.DEPARTMENT = 'ADMINISTRATION'
			AND a.Worklist = '400005'
			THEN 'NON-GOVERNMENTAL BILLING'
		WHEN A.DEPARTMENT = 'ADMINISTRATION'
			AND a.Worklist = '400120'
			THEN 'NON-GOVERNMENTAL BILLING'
		WHEN A.DEPARTMENT = 'CHARGE INTEGRITY'
			AND left(a.BUCKET_INS_CODE, 1) = 'R'
			THEN 'RESEARCH'
		WHEN a.DEPARTMENT IS NULL
			AND Pt_Representative NOT IN ('', '000', '0')
			THEN 'UNMAPPED PATIENT REP NUMBER- REVIEW'
		WHEN A.DEPARTMENT = 'GOVERNMENT BILLING'
			THEN 'GOVERNMENTAL BILLING'
		WHEN A.DEPARTMENT = 'HOSP SVC ZOO ACCTS'
			THEN 'GOVERNMENTAL BILLING'
		WHEN A.DEPARTMENT = 'NGFU-NO WORK'
			THEN 'NON-GOVERNMENTAL FOLLOW UP PENDING'
		WHEN A.DEPARTMENT = 'RED FLAG ACCTS'
			THEN 'GOVERNMENTAL BILLING'
		WHEN A.DEPARTMENT = 'VARIANCE-NO WORK'
			THEN 'VARIANCE UNIT PENDING'
		ELSE A.DEPARTMENT
		END,
	a.[RESPONSIBLE COLLECTOR],
...
SELECT pt_no,
	...
	ifpjoc,
	workable_notworkable_ind = CASE 
		WHEN DEPARTMENT IN ('DENIALS AND APPEALS', 'INSURANCE VERIFICATION', 'NON-GOVERNMENTAL BILLING', 'NON-GOVERNMENTAL FOLLOW UP', 'RESEARCH', 'SELF PAY AND FINANCIAL ASSIST.', 'SPECIAL BILLING', 'VARIANCE UNIT PENDING', 'VENDOR MANAGMENT')
			AND WORKABLE_NOTWORKABLE_IND = 'NOT_WORKABLE'
			THEN 'WORKABLE_PENDING'
		WHEN DEPARTMENT LIKE '%PENDING'
			THEN 'WORKABLE_PENDING'
        WHEN DEPARTMENT = 'SCRUBBER'
            THEN 'WORKABLE'
		ELSE WORKABLE_NOTWORKABLE_IND
		END,
	worklisted,
	ins_buckets,
	bucket_ins_code,
	...
	last_bill_date,
	last_pay_date,
	[department] = LTRIM(RTRIM(REPLACE(department, 'PENDING', ''))), -- remove the word PENDING from the department
