...
DROP TABLE IF EXISTS #c_temp_runout_tbl;
	...
	SELECT A.Pt_No,
		...
		[subsequent_charges] = ISNULL((
				SELECT SUM(E.subsequent_charges)
		...
		[runout_pt_payments] = ISNULL((
				SELECT SUM(E.tot_pmts)
				FROM #c_temp_subseq_pay_allow_rollup AS E
				WHERE E.pay_type = 'Payment'
					AND E.pt_ins_pay = 'Patient'
					AND E.PT_NO = A.PT_NO
					AND ISNULL(E.UNIT_DATE, '') = ISNULL(A.UNIT_DATE, '')
				), 0),
		[runout_ins_payments] = ISNULL((
				SELECT SUM(E.tot_pmts)
				FROM #c_temp_subseq_pay_allow_rollup AS E
				WHERE E.pay_type = 'Payment'
					AND E.pt_ins_pay != 'Patient'
...
DROP TABLE IF EXISTS #unreconciled_tbl;
...
SELECT pt_no,
	...
	[tot_runout_pt_payments] = SUM(ISNULL(runout_pt_payments, 0)),
	[tot_runout_ins_payments] = SUM(ISNULL(runout_ins_payments, 0)),
...
SELECT *
FROM #unreconciled_tbl
WHERE reconciling_variance != 0;
	-- testing
