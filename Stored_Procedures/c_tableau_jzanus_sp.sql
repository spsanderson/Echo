...
-- Create a new stored procedure called 'c_tableau_jzanus_sp' in schema 'dbo'
-- Drop the stored procedure if it already exists
IF EXISTS (
SELECT *
	FROM INFORMATION_SCHEMA.ROUTINES
	WHERE SPECIFIC_SCHEMA = N'dbo'
	AND SPECIFIC_NAME = N'c_tableau_jzanus_sp'
)
DROP PROCEDURE dbo.c_tableau_jzanus_sp
GO
-- Create the stored procedure in the specified schema
CREATE PROCEDURE dbo.c_tableau_jzanus_sp
AS
SET ANSI_NULLS ON
...
BEGIN
	...
	Creates Table/View:
		c_tableau_jzanus_*_tbl
		c_tableau_times_with_jzanus_tbl
	...
	Purpose/Description:
		To build the tables necessary for the Jzanus
		Vendor Dashboard Report in Tableau

	Revision History:
	Date		Version		Description
	----		----		----
	2024-04-10	v1			Initial Creation
	2024-04-22	v2			Added logic for ins/pt payments & IP/OP filter
	************************************************************************/

	DROP TABLE IF EXISTS dbo.c_tableau_jzanus_base_tbl;
	DROP TABLE IF EXISTS dbo.c_tableau_jzanus_stats_tbl;
	DROP TABLE IF EXISTS dbo.c_tableau_jzanus_all_payments_tbl;
	DROP TABLE IF EXISTS dbo.c_tableau_jzanus_payments_tbl;
	DROP TABLE IF EXISTS dbo.c_tableau_times_with_jzanus_tbl;
	DROP TABLE IF EXISTS dbo.c_tableau_jzanus_inventory_tbl;

	CREATE TABLE dbo.c_tableau_jzanus_base_tbl
	(
		c_tableau_jzanus_base_tblId INT IDENTITY(1, 1) NOT NULL PRIMARY KEY, -- primary key column
		pt_no VARCHAR(24),
	...
	SELECT
	    ...
	    pt_rep_description = CASE
	        WHEN pt_rep = '530' THEN 'NEW PLACEMENT'
	        WHEN pt_rep = '531' THEN 'ASSIGNED'
	        WHEN pt_rep = '532' THEN 'CLOSED'
	    END,
	    system_process = 'RPM',
	...
	FROM (
		SELECT
			...
			[pt_rep] = CASE 
				WHEN (CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) IN ('2006','2014','2030','2048','2055','2063','2071','2154') THEN '531'
				WHEN (CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) IN ('2022','2089','2097','2105','2113','2121','2139','2147','2980','2998') THEN '532'
			END,
			[pt_rep_description] = CASE 
				WHEN (CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) IN ('2006','2014','2030','2048','2055','2063','2071','2154') THEN 'ASSIGNED'
				WHEN (CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) IN ('2022','2089','2097','2105','2113','2121','2139','2147','2980','2998') THEN 'CLOSED'
			END,
			(CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) AS [svc_code],
			[PA-ACCT-TYPE] as [acct_type]
		FROM [ECHOLOADERDBP.UHMC.SBUH.STONYBROOK.EDU].[Echo_Active].[dbo].[AccountComments]
		WHERE (CAST([PA-SMART-SVC-CD-WOSCD] AS VARCHAR) + CAST([PA-SMART-SVC-CD-SCD] AS VARCHAR)) IN ('2006','2014','2022','2030','2048','2055','2063','2071','2089','2097','2105','2113','2121','2139','2147','2154','2980','2998')
	...
	VALUES 
	('2006', 'ACCT ASSIGNED TO JZANUS-SP'),
	('2014', 'JZANUS ACCEPTED PLACEMENT-SP'),
	('2022', 'JZANUS DECLINED PLACEMENT'),
	('2030', 'JZANUS Preparing Application'),
	('2048', 'JZANUS Submitted Application'),
	('2055', 'MEDICAID app approved - JZANUS'),
	('2063', 'MEDICAID APRVD W/OVERAGE- JZANUS'),
	('2071', 'MEDICAID APRVD ER ONLY - JZANUS'),
	('2089', 'CLOSED INS identified – JZANUS'),
	('2097', 'CLOSED – DECEASED'),
	('2105', 'Closed – PT Refused App'),
	('2113', 'CLOSED Over Income – JZANUS'),
	('2121', 'CLOSED Over Assets – JZANUS'),
	('2139', 'CLOSED Non-Compliant - JZANUS'),
	('2147', 'CLOSED DISCOVERED MEDICAID'),
	('2154', 'MEDICAID APP PREVIOUSLY SENT'),
	('2980', 'ACCT RECALLED FROM JZANUS'),
	('2998', 'CLOSED WITH JZANUS BY RPM');
	...
	SELECT
		...
		pt_rep_description = CASE
		    WHEN Pt_Representative = '530' THEN 'NEW PLACEMENT'
	        WHEN Pt_Representative = '531' THEN 'ASSIGNED'
	        WHEN Pt_Representative = '532' THEN 'CLOSED'
	...
	WHERE Pt_Representative IN ('530','531','532','  ')
	...
	-- Put the data into the Jzanus base table
	INSERT INTO dbo.c_tableau_jzanus_base_tbl (
		pt_no,
		acct_type,
		pa_smart_date,
		pt_rep,
	...
	Get the balance before and after an account is with Jzanus
	
	----------
	*/

	-- Create a new table called 'c_tableau_times_with_jzanus_tbl' in schema 'dbo'

	-- Create the table in the specified schema
	CREATE TABLE dbo.c_tableau_times_with_jzanus_tbl (
		c_tableau_times_with_jzanus_tblId INT IDENTITY(1, 1) NOT NULL PRIMARY KEY, -- primary key column
		pt_no VARCHAR(255),
		pa_smart_date DATE,
		pt_rep VARCHAR(255),
		pt_rep_description VARCHAR(255),
	...
	WITH CTE_FilteredActivity AS (
		...
		FROM (
			...
			FROM dbo.c_tableau_jzanus_base_tbl A
			WHERE A.pt_rep IN ('531', '532') -- 'ASSIGNED (WITH Jzanus)', 'CLOSED (RETURNED)'
		) FA1
		WHERE FA1.pt_rep <> ISNULL(FA1.PriorActivityNumber, '')
	)

	INSERT INTO dbo.c_tableau_times_with_jzanus_tbl (
		pt_no,
		pa_smart_date,
		pt_rep,
		pt_rep_description,
	...
	LEFT JOIN CTE_FilteredActivity FA2
		...
		AND FA2.pt_rep = '532' -- 'CLOSED (RETURNED)'
	WHERE FA1.pt_rep = '531' -- 'ASSIGNED (WITH Jzanus)'
	...
	SELECT
	    ...
	    M.event_number,
	    M.pa_smart_date AS [with_jzanus] INTO #ACCOUNT_BALANCE_BEFORE
	FROM
	    [ECHOLOADERDBP.UHMC.SBUH.STONYBROOK.EDU].[Echo_Active].[dbo].[DetailInformation] D
	    INNER JOIN SMS.DBO.c_tableau_times_with_jzanus_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	    --INNER JOIN SMS.DBO.c_tableau_jzanus_base_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	WHERE
	    D.[PA-DTL-DATE] <= M.[pa_smart_date]
	    AND M.pt_rep_description = 'ASSIGNED' -- pt_rep 531, account is with Jzanus
	GROUP BY
	...
	SELECT
	    ...
	    M.pt_rep_description,
	    M.event_number,
	    M.pa_smart_date AS [with_jzanus]
	FROM
	...
	SELECT
	    ...
	    M.pt_rep_description,
	    M.event_number,
	    M.pa_smart_date AS [returned_from_jzanus] INTO #ACCOUNT_BALANCE_AFTER
	FROM
	    [ECHOLOADERDBP.UHMC.SBUH.STONYBROOK.EDU].[Echo_Active].[dbo].[DetailInformation] D
	    INNER JOIN SMS.DBO.c_tableau_times_with_jzanus_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	    --INNER JOIN SMS.DBO.c_tableau_jzanus_base_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	...
	SELECT
	    ...
	    M.pt_rep_description,
	    M.event_number,
	    M.pa_smart_date AS [returned_from_jzanus]
	FROM
	    [ECHOLOADERDBP.UHMC.SBUH.STONYBROOK.EDU].[Echo_Archive].[dbo].[DetailInformation] D
	    INNER JOIN SMS.DBO.c_tableau_times_with_jzanus_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	    --INNER JOIN SMS.DBO.c_tableau_jzanus_base_tbl AS M ON M.Pt_No = CAST(D.[PA-PT-NO-WOSCD] AS VARCHAR) + CAST(D.[PA-PT-NO-SCD-1] AS VARCHAR)
	...
	SELECT 
		...
		A.with_jzanus,
		A.balance_before,
		B.returned_from_jzanus,
		B.balance_after
	INTO #BALANCE
	FROM #TEMP_BEFORE_BAL AS A
    OUTER APPLY (
        SELECT TOP 1 B.returned_from_jzanus,
            B.balance_after
	...
	*/
	...
	CREATE TABLE dbo.c_tableau_jzanus_stats_tbl(
		c_tableau_jzanus_stats_tblId INT IDENTITY(1,1) NOT NULL PRIMARY KEY, -- primary key column
		mrn VARCHAR(24),
		pt_no VARCHAR(24),
	...
	INSERT INTO dbo.c_tableau_jzanus_stats_tbl(
		mrn,
		pt_no,
		insurance_code,
		payer_organization,
	...
	SELECT B.MRN AS [mrn],	
		...
		BAL.balance_before,
		BAL.balance_after
	FROM SMS.DBO.c_tableau_jzanus_base_tbl AS A
	...
	LEFT JOIN #BALANCE AS BAL ON A.PT_NO = BAL.PT_NO 
		AND A.pa_smart_date = BAL.with_jzanus;	
	...
	*/
	...
	-- Create the table in the specified schema
	CREATE TABLE dbo.c_tableau_jzanus_all_payments_tbl
	(
	       c_tableau_jzanus_all_payments_tblId INT IDENTITY(1, 1) NOT NULL PRIMARY KEY, -- primary key column
	...
	INSERT INTO dbo.c_tableau_jzanus_all_payments_tbl (pt_no, tot_pay_adj_amt, pay_cd, dtl_type_ind, svc_date, post_date, fin_class, payer_type)
		...
		WHERE (
		       ...
		       AND EXISTS (
		              SELECT 1
		              FROM SMS.dbo.c_tableau_jzanus_base_tbl AS Z
	...
	/*
	Payments while account is with Jzanus
	*/

	-- Make sure we are only getting the records of interest for where an account is with Jzanus
	DROP TABLE IF EXISTS #WITH_JZANUS_TBL;
	SELECT *,
		[partion_number] = ROW_NUMBER() OVER (
			PARTITION BY pt_no ORDER BY pt_no,
				event_number
			)
	INTO #WITH_JZANUS_TBL
	FROM dbo.c_tableau_jzanus_base_tbl
	WHERE pt_rep_description = 'ASSIGNED';
	
	
	CREATE TABLE dbo.c_tableau_jzanus_payments_tbl(
		c_tableau_jzanus_payments_tblId INT IDENTITY(1,1) NOT NULL PRIMARY KEY, -- primary key column
		pt_no VARCHAR(24),
		jzanus_payment DECIMAL(18,2),
		pay_cd VARCHAR(255),
		dtl_type_ind VARCHAR(255),
		svc_date DATE,
		post_date DATE,
	...
	INSERT INTO dbo.c_tableau_jzanus_payments_tbl(
		pt_no,
		jzanus_payment,
		pay_cd,
		dtl_type_ind,
	...
	SELECT M.PT_NO AS [pt_no],
	              M.[tot_pay_adj_amt] AS [jzanus_payment],
	              M.pay_cd,
	              M.dtl_type_ind,
	              M.svc_date,
	              M.post_date,
				  M.fin_class,
				  M.payer_type
	       FROM dbo.c_tableau_jzanus_all_payments_tbl M
	       LEFT JOIN #WITH_JZANUS_TBL AS k ON m.pt_no = k.pt_no
	       WHERE (
	...
	*/
	...
	-- Create a new table called 'c_tableau_jzanus_inventory_tbl' in schema 'dbo'
	
	-- Create the table in the specified schema
	CREATE TABLE dbo.c_tableau_jzanus_inventory_tbl (
		c_tableau_jzanus_inventory_tblId INT IDENTITY(1, 1) NOT NULL PRIMARY KEY, -- primary key column
		inventory_date DATE,
	...
	SET @TODAY = GETDATE();
	SET @STARTDATE = (SELECT EOMONTH(MIN(pa_smart_date)) FROM sms.dbo.c_tableau_times_with_jzanus_tbl);
	...
	INTO #inventory
	FROM dates AS A
	LEFT JOIN sms.dbo.c_tableau_times_with_jzanus_tbl AS B ON B.pa_smart_date <= DATEADD(MONTH, 1, A.DTE)
	       AND ISNULL(B.next_event_date, GETDATE()) >= A.dte
	WHERE A.dte <= @ENDDATE
	GROUP BY A.dte
	ORDER BY A.dte
	...
	-- Put the data into the Jzanus inventory table
	INSERT INTO dbo.c_tableau_jzanus_inventory_tbl (
		inventory_date,
		vendor_inventory
		)
		SELECT *
