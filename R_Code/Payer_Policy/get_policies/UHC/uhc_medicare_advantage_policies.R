...
url <- "https://www.uhcprovider.com/en/policies-protocols/commercial-policies/umr-medical-drug-policies.html"

# Read the HTML content from the page
page <- read_html(url)

# Use html_node() to select the first matching element with the provided XPath
node <- html_node(page, xpath = '//*[@id="columncontiner0-facetedlisting_copy-items"]')
node
...
f_path <- "W:/PATACCT/BusinessOfc/Revenue Cycle Analyst/Payer_Policies/UHCPDFs/Commercial_Plans/UMR_Policies/"
